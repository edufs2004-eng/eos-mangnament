...
export default function ServiciosPage() {
  ...
  // Ajuste automático de porcentajes al cambiar departamento
  const handleDeptChange = (dept) => {
    if (dept === 'LEGAL') {
      ...
      setFormData({
        ...
        departamento: dept,
        pct_caja_empresa: 30,
        pct_ejecutor_legal: 0,
        pct_ejecutor_tech: 70
      });
    } else if (dept === 'MIXTO') {
      setFormData({
        ...formData,
        departamento: dept,
        pct_caja_empresa: 30,
        pct_ejecutor_legal: 35,
        pct_ejecutor_tech: 35
      });
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (totalPct !== 100) {
      alert(`La suma de porcentajes debe ser exactamente 100%. Suma actual: ${totalPct}%`);
      return;
    }
    ...
    try {
      ...
      alert('Error al registrar el servicio en Supabase.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 p-6 md:p-10">
      ...
      <header className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-slate-200 pb-5">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Configuración de prestaciones y asignación automatizada de comisiones por departamento
          </p>
        </div>
      ...
      <section className="mb-8 grid grid-cols-1 gap-5 sm:grid-cols-3">
        <div className="rounded-xl bg-white p-6 shadow-sm border border-slate-200">
          ...
          <div className="mt-4">
            <div className="text-2xl font-extrabold text-slate-900">30% Empresa / 70% Legal</div>
            <p className="mt-1 text-xs text-slate-500">Defensa, contratos, migración, policía local</p>
          </div>
        </div>
        ...
        <div className="rounded-xl bg-white p-6 shadow-sm border border-slate-200">
          ...
          <div className="mt-4">
            <div className="text-2xl font-extrabold text-slate-900">30% Empresa / 70% Tech</div>
            <p className="mt-1 text-xs text-slate-500">Desarrollo web, landing pages, datos</p>
          </div>
        </div>
        ...
        <div className="rounded-xl bg-white p-6 shadow-sm border border-slate-200">
          ...
          <div className="mt-4">
            <div className="text-2xl font-extrabold text-slate-900">30% Empresa / 35% L / 35% T</div>
            <p className="mt-1 text-xs text-slate-500">Blindaje Pyme integral (Legal + Tech)</p>
          </div>
        </div>
      </section>
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-slate-50 text-xs uppercase text-slate-500 border-b border-slate-200">
              <tr>
                ...
                <th className="px-6 py-3 font-semibold">Precio Sugerido</th>
                <th className="px-6 py-3 font-semibold">Caja Empresa</th>
                <th className="px-6 py-3 font-semibold">División Legal</th>
                <th className="px-6 py-3 font-semibold">División Tech</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <tr>
                  <td colSpan="6" className="px-6 py-8 text-center text-slate-500 text-xs">
                    No hay servicios registrados en este departamento.
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={service.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center rounded-md px-2.5 py-1 text-xs font-medium border ${
                        service.departamento === 'LEGAL'
                          ? 'bg-amber-50 text-amber-800 border-amber-200'
                          : service.departamento === 'TECH'
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="Ej: Defensa VIF / Dashboard PowerBI / Blindaje Pyme"
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 uppercase mb-1">Precio Sugerido (CLP)</label>
                  <input
                    type="number"
                    placeholder="150000"
                    value={formData.monto_sugerido}
                    onChange={(e) => setFormData({...formData, monto_sugerido: e.target.value})}
                    className="w-full rounded-lg border border-slate-300 p-2.5 text-sm text-slate-900 outline-none focus:border-blue-600"
                  />
                </div>
              </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 uppercase mb-1">Descripción Breve</label>
                <textarea
                  rows="2"
                  placeholder="Detalles sobre lo que incluye la prestación..."
              ...
              <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-3">
                ...
                <div className="grid grid-cols-3 gap-3 text-center">
                  <div className="bg-white p-2 rounded-lg border border-slate-200">
                    ...
                    <input 
                      type="number" 
                      value={formData.pct_caja_empresa}
                      onChange={(e) => setFormData({...formData, pct_caja_empresa: Number(e.target.value)})}
                      className="w-full text-center font-bold text-blue-900 bg-transparent text-sm"
                    />
                  </div>
                  ...
                  <div className="bg-white p-2 rounded-lg border border-slate-200">
                    ...
                    <input 
                      type="number" 
                      value={formData.pct_ejecutor_legal}
                      onChange={(e) => setFormData({...formData, pct_ejecutor_legal: Number(e.target.value)})}
                      className="w-full text-center font-bold text-amber-700 bg-transparent text-sm"
                    />
                  </div>
                  ...
                  <div className="bg-white p-2 rounded-lg border border-slate-200">
                    ...
                    <input 
                      type="number" 
                      value={formData.pct_ejecutor_tech}
                      onChange={(e) => setFormData({...formData, pct_ejecutor_tech: Number(e.target.value)})}
                      className="w-full text-center font-bold text-teal-700 bg-transparent text-sm"
                    />
                  </div>
                </div>
              </div>
